...
describe('NaturalLanguageParser', () => {
    ...
    test('should correctly interpret "last Friday of the month" as the last Friday of the current month', () => {
        ...
        lastFriday.setDate(lastDayOfMonth.getDate() - ((lastDayOfMonth.getDay() + 2) % 7)); // Calculate the last Friday

        const result = parser.calculateRelativeDate('last Friday of the month');
        console.log('Calculated Date for last Friday of the month:', result.toDateString());
    ...
    test('should correctly interpret "beginning of next month" as the first day of the next month', () => {
        const now = new Date();
        const beginningOfNextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1); // First day of the next month

        const result = parser.calculateRelativeDate('beginning of next month');
        console.log('Calculated Date for beginning of next month:', result.toDateString());
    ...
    describe('Complex Input Parsing', () => {
        test('should parse complex meeting input', () => {
            const result = parser.parse('urgent zoom meeting with John tomorrow morning for 2 hours');
        ...
        test('should parse complex recurring meeting', () => {
            const result = parser.parse('team meeting every monday morning at 9am in the office');
            expect(result).toMatchObject({
                action: 'meet',
        ...
        test('should parse complex call schedule', () => {
            const result = parser.parse('important call with Sarah every day at 10am for 30 minutes');
            expect(result).toMatchObject({
                action: 'call',
                contact: 'Sarah',
                priority: 'high',
    ...
    describe('Enhanced Parsing Features', () => {
        ...
        describe('Reminder Parsing', () => {
            test('should parse custom reminder', () => {
                const result = parser.parse('meeting tomorrow remind me 30 minutes before');
        ...
        describe('Attendee Parsing', () => {
            test('should parse multiple attendees', () => {
                const result = parser.parse('meeting with John, Sarah and Mike tomorrow');
        ...
        describe('Reminder Variations', () => {
            ...
            test('should combine multiple reminders', () => {
                const result = parser.parse('important meeting tomorrow alert me 1 hour before and 10 minutes before');
                expect(result.reminders).toEqual({
